...
namespace Blaze.Server
{
    class LoginCommand
    {
        ...
        {
            var email = (TdfString)request.Data["TOKN"];

            Log.Info(string.Format("Client {0} logging in with Token {1}", request.Client.ID, email.Value));

            // var user = Configuration.Users.Find(u => u.Email == email.Value);
            var user = new User();
            user.Name = "Sinner";
            user.ID = 1;
            user.Email = "Sinnerx9.rx@gmail.com";
            if (user == null)
            {
